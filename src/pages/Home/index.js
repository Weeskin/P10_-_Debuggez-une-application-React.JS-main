...
const Page = () => {
	const { data } = useData(); // remplacement de last par data pour obtenir les données
	const last = data?.events.sort(
		(a, b) => new Date(b.date) - new Date(a.date)
	)[0];
	// last fait le trie sur les dates du plus récent au plus ancien et commence au début du tableau
	return (
		<>
			<header>
				<Menu />
			...
			<main>
				...
				{/* ajout d'une id sur la section pour implémenter une ancre */}
				<section
					id="nos-services"
